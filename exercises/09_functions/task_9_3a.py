    ...
    with open(config_filename, 'r') as conf:
        for line in conf:
            if 'Ethernet'.lower() in line.lower():
                ...
                access_dict.update(
                    {if_name: 1})  # по умолчанию все интерфейсы записываем в словарь аксессных портов с вланом 1
                ...
                access_dict.update(
                    {if_name: acc_vlan})  # методом update обновляем интерфейс с нужным вланом вместо дефолтного 1
